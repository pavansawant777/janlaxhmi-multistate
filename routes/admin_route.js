...
router.get("/home",verifyUrl, async function(req,res){
    ...
    {
        var data = await exe(`SELECT * FROM company`);
    var data1 = await exe(`SELECT COUNT(*) as no_of_enquires FROM enquiry WHERE enquiry_status='uncomplete' `);
    var data2 = await exe(`SELECT COUNT(*) as no_of_enquires FROM service_enquiry WHERE status = 'uncomplete' `);
    var data3 = await exe(`SELECT COUNT(*) as no_of_enquires FROM loan_enquiry WHERE enquiry_status='incomplete' `);
    var data4 = await exe(`SELECT COUNT(*) as no_of_enquires FROM current_deposit_enquiry WHERE enquiry_status='incomplete'`);
    var data5 = await exe(`SELECT COUNT(*) as no_of_enquires FROM contact_enquiry WHERE enquiry_status='incomplete'`);
...
router.get("/service_enquiry",async function(req,res)
{
    ...
    var total_records = (await exe(`SELECT COUNT(*) as enquiry_pages FROM service_enquiry WHERE status='uncomplete'`))[0].enquiry_pages;
    per_page = 5;
    ...
    var data= await exe(`SELECT * FROM company`);
    var data2=await exe(`SELECT * FROM service_enquiry WHERE status='uncomplete' LIMIT ${starting_index_of_limit}, ${per_page}`);

    var obj={"s_enquirys":data2,"company_info":data[0], "is_login":checklogin(req), "total_pages":total_pages,
        "page_no":page_no};
    res.render("admin/service_enquiry.ejs",obj);
})


router.get("/completed_service_enquiry",async function (req,res) 
{
    var urldata = url.parse(req.url,true).query;
    var total_records = (await exe(`SELECT COUNT(*) as enquiry_pages FROM service_enquiry WHERE status='complete'`))[0].enquiry_pages;
    per_page = 5;
    total_pages = total_records / per_page;
    if(parseInt(total_pages)<total_pages)
    {
    ...
    starting_index_of_limit = (per_page * page_no) - per_page;
    var data= await exe(`SELECT * FROM company`);
    var data3=await exe(`SELECT * FROM service_enquiry WHERE status='complete' LIMIT ${starting_index_of_limit}, ${per_page} `);
    var obj={"company_info":data[0], "co_s_enquiry":data3 , "is_login":checklogin(req),"total_pages":total_pages,
        "page_no":page_no};
    res.render("admin/completed_service_enquiry.ejs",obj);
    
})

router.get("/completed_service_enquiry_delete/:service_enquiry_id",async function(req,res)
{
    var id = req.params.service_enquiry_id;
    var sql = `DELETE FROM service_enquiry WHERE service_enquiry_id = '${id}'`;
    var data = await exe(sql);
    res.redirect("/admin/completed_service_enquiry");
});
...
router.get("/enquiry",async function(req,res){
    var urldata = url.parse(req.url,true).query;
    var total_records = (await exe(`SELECT COUNT(*) as enquiry_pages FROM enquiry WHERE enquiry_status='uncomplete'`))[0].enquiry_pages;
    per_page = 5;
    total_pages = total_records / per_page;
    if(parseInt(total_pages)<total_pages)
    {
    ...
    starting_index_of_limit = (per_page * page_no) - per_page;
    var sql=`SELECT * FROM enquiry Where enquiry_status='uncomplete' LIMIT ${starting_index_of_limit}, ${per_page} `;
    var data= await exe(sql);
    var obj={"enquirys":data,"is_login":checklogin(req),"total_pages":total_pages,
        "page_no":page_no};
...
router.get("/completed_home_enquiry",async function(req,res){

    var urldata = url.parse(req.url,true).query;
    var total_records = (await exe(`SELECT COUNT(*) as enquiry_pages FROM enquiry WHERE enquiry_status='complete'`))[0].enquiry_pages;
    per_page = 5;
    total_pages = total_records / per_page;
    if(parseInt(total_pages)<total_pages)
    {
    ...
    var data2=await exe(`SELECT * FROM enquiry Where enquiry_status='complete' LIMIT ${starting_index_of_limit}, ${per_page} `)
    var obj={"co_enquiry":data2,"is_login":checklogin(req),"total_pages":total_pages,
        "page_no":page_no};
    res.render("admin/completed_home_enquiry.ejs",obj);
});
...
router.get("/completed_enquiry_delete/:enquiry_id",function(req,res){
    var id=req.params.enquiry_id;
    var sql=`DELETE FROM enquiry WHERE enquiry_id='${id}' `;
    var data =exe(sql);
    res.redirect("/admin/completed_home_enquiry");
});
...
router.get("/complete_service_enquiry/:service_enquiry_id",function(req,res){
    ...
    // res.send(req.body);
    var data=exe(sql);
    res.redirect("/admin/service_enquiry");
});


router.get("/completed_service_enquiry_delete/:service_enquiry_id",function(req,res){
    var id=req.params.service_enquiry_id;
    var sql=`DELETE FROM service_enquiry WHERE service_enquiry_id='${id}' `     
    var data=exe(sql);
    res.redirect("/admin/service_enquiry")
...
{
    var urldata = url.parse(req.url,true).query;
    var total_records = (await exe(`SELECT COUNT(*) as enquiry_pages FROM loan_enquiry`))[0].enquiry_pages;
    per_page = 5;
    total_pages = total_records / per_page;
    if(parseInt(total_pages)<total_pages)
    {
    ...
    var sql = `SELECT * FROM loan_enquiry WHERE enquiry_status = 'incomplete' LIMIT ${starting_index_of_limit}, ${per_page}`;
    var data = await exe(sql);
    var obj = {"total_pages":total_pages,
        "page_no":page_no,
...
router.get("/complete_loan_enquiry/:loan_enquiry_id",async function(req,res)
{
    var id = req.params.loan_enquiry_id;
    var sql = await exe(`UPDATE loan_enquiry SET enquiry_status = 'complete' WHERE loan_enquiry_id = '${id}'`)
    res.redirect("/admin/loan_enquiry")
});


router.get("/completed_loan_enquiry",async function(req,res) 
{
    var urldata = url.parse(req.url,true).query;
    var total_records = (await exe(`SELECT COUNT(*) as enquiry_pages FROM loan_enquiry WHERE enquiry_status = 'complete'`))[0].enquiry_pages;
    per_page = 5;
    total_pages = total_records / per_page;
    if(parseInt(total_pages)<total_pages)
    {
    ...
    var data= await exe(`SELECT * FROM company`);
    var data1= await exe(`SELECT * FROM loan_enquiry WHERE enquiry_status = 'complete' LIMIT ${starting_index_of_limit}, ${per_page}`);
    var obj = {"total_pages":total_pages,
        "page_no":page_no,
        "is_login":checklogin(req),
        "company_info":data[0],
        "completed_enquiry":data1
    }
    res.render("admin/completed_loan_enquiry.ejs",obj)
});
...
{
    var urldata = url.parse(req.url,true).query;
    var total_records = (await exe(`SELECT COUNT(*) as enquiry_pages FROM contact_enquiry WHERE enquiry_status = 'incomplete'`))[0].enquiry_pages;
    per_page = 5;
    total_pages = total_records / per_page;
    if(parseInt(total_pages)<total_pages)
    {
    ...
    starting_index_of_limit = (per_page * page_no) - per_page;
    var sql = `SELECT * FROM contact_enquiry WHERE enquiry_status = 'incomplete' LIMIT ${starting_index_of_limit}, ${per_page}`;
    var data = await exe(sql);
    var obj = {"contact_enquiry":data,"is_login":checklogin(req),"total_pages":total_pages,
        "page_no":page_no};
    res.render("admin/contact_enquiry.ejs", obj);
});


router.get("/completed_contact_enquiry",async function(req,res)
{
    var urldata = url.parse(req.url,true).query;
    var total_records = (await exe(`SELECT COUNT(*) as enquiry_pages FROM contact_enquiry WHERE enquiry_status = 'complete'`))[0].enquiry_pages;
    per_page = 5;
    total_pages = total_records / per_page;
    if(parseInt(total_pages)<total_pages)
    {
    ...
    starting_index_of_limit = (per_page * page_no) - per_page;
    var sql = `SELECT * FROM contact_enquiry WHERE enquiry_status = 'complete' LIMIT ${starting_index_of_limit}, ${per_page}`;
    var data = await exe(sql);
    var obj = {"contact_enquiry":data,"is_login":checklogin(req),"total_pages":total_pages,
        "page_no":page_no};
    res.render("admin/completed_contact_enquiry.ejs", obj);
});

router.get("/complete_contact_enquiry/:contact_enq_id",async function(req,res) 
{
    var id = req.params.contact_enq_id;
    var sql = await exe(`UPDATE contact_enquiry SET enquiry_status = 'complete' WHERE contact_enq_id = '${id}'`)
...
router.get("/delete_contact_enquiry_details/:contact_enq_id",async function(req,res)
{
    ...
    var data = await exe(sql);
    res.redirect("/admin/completed_contact_enquiry");
});
...
router.get("/delete_current_deposit_enquiry/:current_deposit_enquiry_id",async function(req,res)
{
    ...
    var data = await exe(sql);
    res.redirect("/admin/completed_deposit_enquiry");
});


router.get("/current_deposite_enquiry", async function(req,res){
    var urldata = url.parse(req.url,true).query;
    var total_records = (await exe(`SELECT COUNT(*) as enquiry_pages FROM current_deposit_enquiry WHERE enquiry_status = 'incomplete'`))[0].enquiry_pages;
    per_page = 5;
    total_pages = total_records / per_page;
    if(parseInt(total_pages)<total_pages)
    {
    ...
    var data = await exe(`SELECT * FROM company`);
    var data3 = await exe(`SELECT * FROM current_deposit_enquiry WHERE enquiry_status = 'incomplete' LIMIT ${starting_index_of_limit}, ${per_page}`)
    var obj={"company_info":data[0],
            "is_login":checklogin(req),
            "current_deposit_enquiry":data3,"total_pages":total_pages,
        "page_no":page_no}
...
router.get("/completed_deposit_enquiry",async function (req,res)
{
    var urldata = url.parse(req.url,true).query;
    var total_records = (await exe(`SELECT COUNT(*) as enquiry_pages FROM current_deposit_enquiry WHERE enquiry_status = 'complete'`))[0].enquiry_pages;
    per_page = 5;
    total_pages = total_records / per_page;
    if(parseInt(total_pages)<total_pages)
    {
    ...
    starting_index_of_limit = (per_page * page_no) - per_page;
    var data = await exe(`SELECT * FROM company`);
    var data1 = await exe(`SELECT * FROM current_deposit_enquiry WHERE enquiry_status = 'complete' LIMIT ${starting_index_of_limit}, ${per_page}`)
    var obj = {"company_info":data[0],
            "is_login":checklogin(req),
            "total_pages":total_pages,
            "current_deposit_enquiry":data1,
        "page_no":page_no
    }
    res.render("admin/completed_deposit_enquiry.ejs",obj)
});



router.get("/complete_deposit_enquiry/:current_deposit_enquiry_id",async function(req,res) 
{
    var id = req.params.current_deposit_enquiry_id;
    var data = await exe (`UPDATE current_deposit_enquiry SET enquiry_status = 'complete' WHERE current_deposit_enquiry_id = '${id}'`);
